...
void combineHistos(string type) {
    ...
    for (int itrg = 0; itrg != _jp_ntrigger; ++itrg) {
        _ptranges[_jp_triggers[itrg]] = pair<double, double>(
            _jp_trigranges[itrg][0], _jp_trigranges[itrg][1]);
    }
    ...
    std::cout << std::endl;
    std::cout << "Output stored in " << fout->GetName() << std::endl;
    fout->Close();
    fout->Delete();
...
TH1D *recurseFile(TDirectory *indir, TDirectory *outdir, string hname,
    ...
    while ((key = itkey.Next())) {
        ...
        if (classname == "TDirectoryFile" && obj->InheritsFrom("TDirectory")) {
            ...
            float etamin, etamax;
            if ((sscanf(indir2->GetName(), "Eta_%f-%f", &etamin, &etamax) ==
                 2) &&
                (etamax > etamin)) {
                ...
                _hpt =
                    recurseFile(indir2, outdir2, hname, atBottom, _hpt, etamid);
            }
        ...
        if (kname == hname && obj->InheritsFrom("TProfile3D")) {
            if (_hpt == 0) {
                outdir->cd();
                _hpt =
                    (TH1D *)((TProfile3D *)obj)->ProjectionXYZ(obj->GetName());
                indir->cd();
            }
        } // TProfile3D
        ...
        if (kname == hname && obj->InheritsFrom("TH3")) {
            ...
            if (_hpt == 0) {
                ...
                assert(_hpt);
                if (_debug)
                    cout << "Cloned _" << hpt3->GetName() << endl;
            }

            if (_ptranges.find(indir->GetName()) == _ptranges.end())
                cout << "pT range not found for directory " << indir->GetName()
                     << endl;
                ...
                if (pt > ptmin && pt < ptmax) {
                    for (int j = 1; j != _hpt3->GetNbinsY() + 1; ++j) {
                        for (int k = 1; k != _hpt3->GetNbinsZ() + 1; ++k) {
                            _hpt3->SetBinContent(i, j, k,
                                                 hpt3->GetBinContent(i, j, k));
                            _hpt3->SetBinError(i, j, k,
                                               hpt3->GetBinError(i, j, k));
                        } // for l
                    }     // for j
                }         // in ptrange
            }             // for i
            ...
            if (_hpt == 0) {
                ...
                assert(_hpt);
                if (_debug)
                    cout << "Cloned _" << hpt2->GetName() << endl;
            }
